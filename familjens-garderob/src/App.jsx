...
export default function App() {
    ...
    if (user) {
        if (appData) {
            return <WardrobeManager key={user.uid} user={user} appData={appData} />;
        }
...
function AuthGate({ onLogin }) {
    ...
    if (isNewUser) {
        return <ProfileSetup user={newUser} />;
    }

    return (
        <div className="flex items-center justify-center h-screen bg-gray-50">
            <div className="w-full max-w-sm p-8 space-y-6 bg-white rounded-lg shadow-md">
                <h2 className="text-2xl font-bold text-center">Välkommen tillbaka!</h2>
...
// Den fullständiga koden finns i appen, men förkortas här för tydlighetens skull.
