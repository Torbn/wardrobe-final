...
export default function App() {
            ...
            try {
                ...
                onAuthStateChanged(auth, (currentUser) => {
                    ...
                    if (!currentUser && !sessionId) {
                        // Endast om ingen är inloggad OCH det inte finns ett session ID
                        signInAnonymously(auth)
                            ...
                            .catch(err => setError("Autentisering misslyckades."));
                    } else {
                        setLoading(false);
                    }
                });
    ...
    useEffect(() => {
        if (!user) {
            setLoading(false); // Om användaren loggas ut, sluta ladda
            setAppData(null); // Rensa gammal data
            return;
        };
    ...
    return <SkeletonLoader />; // Fallback om ingen annan vy renderas
}
...
function ProfileSetup({ user, onJoinRequest, joinFamilyIdFromUrl }) {
        ...
        <>
            ...
            <div className="flex items-center justify-center h-screen">
                <div className="bg-white p-8 rounded-lg shadow-xl max-w-sm w-full text-center">
                    ...
                    <button onClick={handleSetup} disabled={!name || isProcessing} className="w-full bg-green-500 text-white p-3 rounded-lg font-semibold hover:bg-green-600 disabled:bg-gray-400">
                        Skapa garderob
                    </button>
                </div>
            </div>
